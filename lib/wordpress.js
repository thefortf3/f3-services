...
async function getIdBySearch(type, query) {
    let url = config.wordpress.base_url + type + "?per_page=100&search="
    ...
    if (response_json.length > 0) {
        for (let result of response_json) {
            console.log("Comparing " + result['name'].toLowerCase() + " to " + query.toLowerCase())
...
wordpress.postToWordpress = async function(title, date, qic, coq, ao, paxlist, backblast, preblast=false, noPost=false) {
    // Check for NO_POST flag from environment or parameter
    const skipPosting = noPost || process.env.NO_POST === 'true' || process.env.NO_POST === '1';
    
    if (skipPosting) {
        console.log('\n=== NO_POST MODE - Simulating WordPress post ===');
        console.log('Title:', title);
        console.log('Date:', date);
        console.log('QIC:', qic);
        console.log('COQ:', coq);
        console.log('AO:', ao);
        console.log('Preblast:', preblast);
        console.log('Content length:', backblast?.length || 0);
    }
    ...
    if (preblast) {
        let pb_id = await getIdBySearch('categories', "Pre-Blast")
        if (pb_id == null) {
            throw new Error("Unable to find pre-blast category")
        }
        let new_ids = []
        new_ids.push(pb_id)
        new_ids.push(ao_id)
    ...
    // Skip actual WordPress posting if NO_POST flag is set
    if (skipPosting) {
        console.log('=== Skipping WordPress API call (NO_POST mode) ===\n');
        return {
            id: 'mock-' + Date.now(),
            title: { rendered: title },
            link: 'http://example.com/mock-post',
