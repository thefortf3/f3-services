...
app.action('commit_event', async ({ ack, body, client, logger }) => {
  ...
  try {
    ...
    const blockedUserIds = blockedUsers.split(',').map(id => id.trim()).filter(id => id);
        ...
        blocks: [
          {
            ...
            text: {
              type: 'mrkdwn',
              text: '*Ah ah ah! You didn\'t say the magic word!* :dinosaur:'
            }
          },
          {
            type: 'image',
            image_url: 'https://media.giphy.com/media/v1.Y2lkPTc5MGI3NjExcjN3dGMxanoyNzJqZ3V6ZmR4YWh5MHY2cTl5dmxvdmFyaXcyNWZuYyZlcD12MV9naWZzX3NlYXJjaCZjdD1n/3orifgVJ5OyzYv9Jvy/giphy.gif',
    ...
    for (let line of lines) {
      if (line.includes('*Committed:*')) {
        ...
        // Extract current count for display
        const currentMentions = line.match(/<@[A-Z0-9]+>/g) || [];
        const newCount = currentMentions.length + 1;
        
        if (line.includes('None yet')) {
          newLines.push('*Committed:* ' + mention + ` (${newCount})`);
        } else {
          // Remove old count if present
          const lineWithoutCount = line.replace(/\s*\(\d+\)$/, '');
          newLines.push(lineWithoutCount + ' ' + mention + ` (${newCount})`);
        }
      } else {
        newLines.push(line);
      }
  ...
  try {
    ...
    for (let line of lines) {
      if (line.includes('*Committed:*')) {
        ...
        const filtered = parts.filter(p => p !== mention && !p.match(/^\(\d+\)$/));  // Also remove count
        
        // Count remaining mentions
        const remainingMentions = filtered.filter(p => p.match(/<@[A-Z0-9]+>/));
        const newCount = remainingMentions.length;
        ...
        if (newCount === 0) {
          ...
          newLines.push(filtered.join(' ') + ` (${newCount})`);
        }
      } else {
        newLines.push(line);
      }
